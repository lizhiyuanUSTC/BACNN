...
tf.app.flags.DEFINE_string('eval_data', '/media/ai/DL_DATA/HCCR/THRESH_OTSU_aug/competition.tfrecords',
                           """Either 'test' or 'train_eval'.""")
tf.app.flags.DEFINE_integer('eval_interval_secs', 60 * 5,
                            """How often to run the eval.""")
tf.app.flags.DEFINE_integer('num_examples', 2019771,
    ...
    with tf.Session() as sess:
      ...
      try:
        ...
        while step < num_iter + 1 and not coord.should_stop():
          ...
          print('%s: Model %d, Batch %d, precision @ 1 = %.3f' % (datetime.now(), model_count, step, 9*np.sum(predictions) / FLAGS.eval_batch_size))
          if step == num_iter:
        ...
        # Compute precision @ 1.
        precision = 9 * true_count / FLAGS.num_examples
        acc.append(precision)
        print('%d / %d' % (true_count, FLAGS.num_examples / 9))
 ...
 # FLAGS.batch_szie = 1000
  with tf.Graph().as_default() as g:
    ...
    if FLAGS.is_aug:
      ...
      logits = tf.reshape(logits, [-1, 9, FLAGS.NUM_CLASSES])
      logits = tf.unstack(logits, 9, axis=1)
      prob = tf.zeros([FLAGS.eval_batch_size / 9, FLAGS.NUM_CLASSES])
      for s in logits:
        prob += s
      logits = prob / 9
      labels = labels[::9]
